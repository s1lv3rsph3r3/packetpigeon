resource "digitalocean_droplet" "blue" {
  count = lookup(local.deploying_env[var.deployment_environment].blue[var.deployment_stage], "server", 0) ? 1 : 0
...
resource "cloudflare_record" "packetpigeon_blue" {
  ...
  ttl = 120
  count = lookup(local.deploying_env[var.deployment_environment].blue[var.deployment_stage], "dns", 0) ? 1 : 0
}
